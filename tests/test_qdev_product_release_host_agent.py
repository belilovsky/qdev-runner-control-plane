...
def test_qgeo_materialization_cleans_abandoned_staging_on_idempotent_retry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile = replace(
        _qgeo_profile(tmp_path),
    ...
    def fake_run(command: list[str], **_kwargs: object) -> bytes:
        ...
        if command[:2] == ["docker", "cp"]:
            destination = Path(command[-1])
            (destination / "app.css").write_text("candidate", encoding="utf-8")
            return b""
        if command[:3] == ["docker", "rm", "--force"]:
            return b""
        raise AssertionError(command)
    ...
    monkeypatch.setattr(AGENT, "_run", fake_run)
    expected = AGENT.materialize_static(release, profile)
    abandoned = profile.static_directory_root / f".{SHA}.abandoned"
    (abandoned / "nested").mkdir(parents=True)
    (abandoned / "nested" / "partial.css").write_text("partial", encoding="utf-8")

    assert AGENT.materialize_static(release, profile) == expected
    assert not abandoned.exists()


def test_qgeo_materialization_rejects_existing_static_tree_that_differs_from_image(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile = replace(
        _qgeo_profile(tmp_path),
    ...
    monkeypatch.setattr(AGENT, "_run", fake_run)
    with pytest.raises(AGENT.AgentError, match="does not match candidate"):
        AGENT.materialize_static(release, profile)


@pytest.mark.parametrize("orphan", ["tree", "manifest"])
def test_qgeo_materialization_recovers_each_interrupted_publish_boundary(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, orphan: str
) -> None:
    profile = replace(
        _qgeo_profile(tmp_path),
        static_directory_root=tmp_path / "static",
    ...
    target = profile.static_directory_root / SHA
    assert target is not None
    manifest_path = profile.static_directory_root.parent / "manifests" / f"{SHA}.json"
    if orphan == "tree":
        target.mkdir(parents=True)
        (target / "app.css").write_text("candidate", encoding="utf-8")
    else:
        fixture = tmp_path / "fixture"
        fixture.mkdir()
        (fixture / "app.css").write_text("candidate", encoding="utf-8")
        bundle, digest = AGENT._static_manifest(fixture, release)
        manifest_path.parent.mkdir()
        manifest_path.write_text(
            json.dumps(AGENT._static_manifest_document(bundle, digest)), encoding="utf-8"
        )
    ...
    monkeypatch.setattr(AGENT, "_run", fake_run)
    recovered = AGENT.materialize_static(release, profile)
    assert recovered is not None
    assert target.is_dir()
    assert manifest_path.is_file()
    assert not list(profile.static_directory_root.glob(f".{SHA}.*"))
    assert AGENT.materialize_static(release, profile) == recovered


def test_qgeo_materialization_syncs_tree_before_publishing_manifest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile = replace(
        _qgeo_profile(tmp_path),
    ...
    original_sync = AGENT._fsync_static_tree
    original_install = AGENT._install_static_manifest

    def observed_sync(directory: Path) -> None:
        events.append(f"sync:{directory.name}")
        original_sync(directory)

    def observed_install(path: Path, manifest: dict[str, object]) -> None:
        events.append("manifest")
        original_install(path, manifest)

    monkeypatch.setattr(AGENT, "_run", fake_run)
    monkeypatch.setattr(AGENT, "_fsync_static_tree", observed_sync)
    monkeypatch.setattr(AGENT, "_install_static_manifest", observed_install)

    AGENT.materialize_static(release, profile)

    assert events[0].startswith("sync:.")
    assert events[1] == "manifest"


def test_qgeo_materialization_resyncs_publication_directories_on_retry(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    profile = replace(
        _qgeo_profile(tmp_path),
        static_directory_root=tmp_path / "release" / "static",
        rollback_static_directory=tmp_path / "rollback",
    )
    release = {
        "source_sha": SHA,
    ...
    synced: list[Path] = []
    original_sync = AGENT._fsync_directory

    def observed_sync(directory: Path) -> None:
        synced.append(directory)
        original_sync(directory)

    monkeypatch.setattr(AGENT, "_fsync_directory", observed_sync)
    AGENT.materialize_static(release, profile)
    ...
    assert root.parent / "manifests" in synced


def test_qgeo_durable_directory_creation_syncs_each_parent(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(AGENT, "_fsync_directory", observed_sync)
    AGENT._mkdir_durable(target)

    assert target.is_dir()
    assert synced == [tmp_path.parent, tmp_path, tmp_path / "one"]


def test_qgeo_durable_directory_creation_resyncs_interrupted_existing_ancestor(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    interrupted = tmp_path / "one"
    interrupted.mkdir()
    target = interrupted / "two"
    synced: list[Path] = []
    original_sync = AGENT._fsync_directory
    ...
    assert target.is_dir()
    assert synced == [tmp_path, interrupted]
