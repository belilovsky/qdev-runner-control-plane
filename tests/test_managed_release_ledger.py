...
def test_qazgeo_ledger_classifies_fifo_admission_without_mutation(tmp_path: Path) -> None:
    ...
    document["entries"]["qazgeo"]["ci_runs"][0]["conclusion"] = "success"
    ...
    assert terminal.classify_admission("qazgeo", PR_CHECKOUT_SHA, run_id=run_id, run_attempt=1) == (
        False,
        "managed-production-candidate-tuple-not-admitted",
    )
    with pytest.raises(ManagedReleaseLedgerError, match="terminal"):
