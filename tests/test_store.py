...
def test_store_repairs_null_legacy_keys_without_rewriting_accepted_values(
    tmp_path: Path,
) -> None:
    database = tmp_path / "broker.db"
    store = Store(database)
    assert store.enqueue(job(queued_at="2026-08-27T13:46:59Z")) is True
    accepted = store.job(100)
    assert accepted is not None
    with store.connect() as connection:
        connection.execute("DROP TRIGGER jobs_queue_key_immutable")
        connection.execute(
            "UPDATE jobs SET queue_time_source=NULL, required_profile=NULL WHERE job_id=100"
        )
        connection.execute(
            """
    ...
    assert (repaired["github_queued_at"], repaired["queue_sequence"]) == (
        accepted["github_queued_at"],
        accepted["queue_sequence"],
    )
    assert repaired["queue_time_source"] == "legacy_preserved_timestamp"
    assert repaired["required_profile"] == accepted["required_profile"]
