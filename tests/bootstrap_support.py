"""Synthetic bootstrap identities; never production credentials or receipts."""

from __future__ import annotations

import io
import json
import tarfile
import time
from pathlib import Path
from typing import Any

from qdev_runner.bootstrap_authority import authorize_bootstrap
from qdev_runner.controller_image_candidate import create as create_candidate_receipt
from qdev_runner.fleet_bootstrap import REQUEST_SCHEMA, FleetBootstrapPolicy, FleetBootstrapRequest
from qdev_runner.models import QueuedJob
from qdev_runner.store import Store

ROOT = Path(__file__).resolve().parents[1]
KEY = "synthetic-directive-key-" + "x" * 32


def candidate_receipt(
    *, source_revision: str = "a" * 40, image_digest: str = "sha256:" + "b" * 64
) -> dict[str, Any]:
    return create_candidate_receipt(
        source_revision=source_revision,
        bundle_digest="d" * 64,
        image_digest=image_digest,
        base_image_digest="sha256:" + "e" * 64,
        wheelhouse_digest="f" * 64,
        requirements_digest="1" * 64,
        sbom_digest="2" * 64,
        run_id="777",
        run_attempt=2,
        job="controller-image-candidate",
        created_at="2026-09-04T12:00:00Z",
    )


def candidate_artifact(
    root: Path,
    receipt: dict[str, Any],
    *,
    repository: str | None = None,
    job_id: int = 778,
) -> Path:
    repository = repository or policy().identity.repository
    owner, repo = repository.split("/", 1)
    source = str(receipt["source_revision"])
    attempt = int(receipt["run_attempt"])
    target = (
        root
        / owner
        / repo
        / source
        / str(job_id)
        / f"controller-image-candidate-{source}-{attempt}.tar.gz"
    )
    target.parent.mkdir(parents=True, exist_ok=True)
    payload = json.dumps(receipt, ensure_ascii=True, sort_keys=True, separators=(",", ":"))
    body = payload.encode() + b"\n"
    info = tarfile.TarInfo("controller-image-candidate-receipt.json")
    info.size = len(body)
    info.mtime = 0
    info.uid = 0
    info.gid = 0
    with tarfile.open(target, mode="w:gz") as archive:
        archive.addfile(info, io.BytesIO(body))
    return target


def policy() -> FleetBootstrapPolicy:
    return FleetBootstrapPolicy(
        ROOT / "config/fleet-bootstrap.yml",
        ROOT / "config/release-lanes.yml",
    )


def request(worker: str = "srv1879763-primary") -> FleetBootstrapRequest:
    return FleetBootstrapRequest.model_validate(
        {
            "schema": REQUEST_SCHEMA,
            "action": "restore-existing-worker",
            "source_sha": "a" * 40,
            "run_id": 123,
            "job_id": 456,
            "attempt": 1,
            "claim_ttl_seconds": 300,
            "controller_revision": "a" * 40,
            "controller_release_digest": "sha256:" + "b" * 64,
            "release_lane": None,
            "worker_name": worker,
        }
    )


def claims() -> dict[str, Any]:
    identity = policy().identity
    now = int(time.time())
    return {
        "iss": "https://token.actions.githubusercontent.com",
        "aud": identity.audience,
        "repository": identity.repository,
        "ref": "refs/heads/main",
        "sha": "a" * 40,
        "run_id": "123",
        "run_attempt": "1",
        "event_name": "workflow_dispatch",
        "workflow_ref": f"{identity.repository}/{identity.workflow}@refs/heads/main",
        "iat": now,
        "nbf": now,
        "exp": now + 300,
    }


class Verifier:
    def __init__(self) -> None:
        self.claims = claims()

    def verify_and_decode(self, token: str, **kwargs: Any) -> dict[str, Any]:
        assert token == "synthetic-oidc"  # noqa: S105 - inert fixture
        assert kwargs == {
            "repository": policy().identity.repository,
            "sha": "a" * 40,
            "run_id": 123,
        }
        return self.claims


class GitHub:
    def __init__(self) -> None:
        self.job = {
            "id": 456,
            "run_id": 123,
            "run_attempt": 1,
            "head_sha": "a" * 40,
            "status": "in_progress",
            "name": "fleet-bootstrap-execute",
        }
        self.run = {
            "id": 123,
            "run_attempt": 1,
            "head_sha": "a" * 40,
            "head_branch": "main",
            "path": policy().identity.workflow,
            "event": "workflow_dispatch",
            "status": "in_progress",
        }
        self.candidate_run = {
            "id": 777,
            "run_attempt": 2,
            "head_sha": "a" * 40,
            "head_branch": "main",
            "path": ".github/workflows/controller-image-candidate.yml",
            "event": "workflow_dispatch",
            "status": "completed",
            "conclusion": "success",
        }
        self.candidate_jobs = [
            {
                "id": 778,
                "run_id": 777,
                "run_attempt": 2,
                "head_sha": "a" * 40,
                "status": "completed",
                "conclusion": "success",
                "name": "controller-image-candidate",
            }
        ]

    def workflow_job(self, installation: int, repository: str, job_id: int) -> dict[str, Any]:
        assert (installation, repository, job_id) == (300, policy().identity.repository, 456)
        return self.job

    def workflow_run(self, installation: int, repository: str, run_id: int) -> dict[str, Any]:
        assert installation == 300 and repository == policy().identity.repository
        if run_id == 123:
            return self.run
        assert run_id == 777
        return self.candidate_run

    def workflow_jobs(
        self, installation: int, repository: str, run_id: int, attempt: int
    ) -> list[dict[str, Any]]:
        assert (installation, repository, run_id, attempt) == (
            300,
            policy().identity.repository,
            777,
            2,
        )
        return self.candidate_jobs


def register(store: Store, worker: str = "srv1879763-primary", active_jobs: int = 0) -> None:
    target = policy().worker_target(worker)
    store.enqueue(
        QueuedJob(
            "bootstrap",
            456,
            123,
            policy().identity.repository,
            1,
            300,
            ("self-hosted", "Linux", "X64", "qdev-ci"),
            "a" * 40,
            "main",
            {},
        )
    )
    assert store.claim("bootstrap-executor", ("qdev-ci",)) is not None
    store.heartbeat(
        worker,
        ("qdev-ci", "qdev-ci-browser", "qdev-ci-docker"),
        active_jobs,
        (),
        {
            "tier": "primary",
            "authenticated_certificate_sha256": (
                target.certificate_fingerprint_sha256 if target is not None else None
            ),
        },
    )


def operation(store: Store, req: FleetBootstrapRequest, key: str = KEY) -> Any:
    artifact_root = store.path.parent / "artifacts"
    if req.action == "activate-controller":
        assert req.controller_candidate_receipt is not None
        candidate_artifact(artifact_root, req.controller_candidate_receipt)
    return authorize_bootstrap(
        token="synthetic-oidc",
        request=req,
        idempotency_key="worker-recovery-001",
        policy=policy(),
        verifier=Verifier(),
        github=GitHub(),  # type: ignore[arg-type]
        controller_store=store,
        artifact_root=artifact_root,
        signing_key=key,
    )
