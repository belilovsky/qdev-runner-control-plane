...
def test_qazposter_contract_admission_is_repository_scoped() -> None:
    ...
    assert policy.repository_profile_disk_mb[("belilovsky/qazposter", "qdev-ci")] == 4 * 1024
...
def test_controller_verification_admission_is_repository_scoped() -> None:
    ...
    assert (
        policy.repository_profile_disk_mb[("belilovsky/qdev-runner-control-plane", "qdev-ci")]
        == 4 * 1024
    )
    assert policy.profiles["qdev-ci"].disk_mb == 12 * 1024
...
def test_platform_portal_contract_admission_is_repository_scoped() -> None:
    ...
    assert policy.repository_profile_disk_mb[("belilovsky/platform-portal", "qdev-ci")] == 4 * 1024
    assert policy.profiles["qdev-ci"].disk_mb == 12 * 1024
