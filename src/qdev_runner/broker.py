    ...
    *,
    ...
    def is_registered_test_workflow(job: dict[str, Any]) -> bool:
        ...
        explicitly_registered = (
            repo.workflow_registration_present and registration is not None
        )
        ...
        *args: Any,
        ...
        if isinstance(token_or_request, Request):
            ...
            if settings.operator_proxy_secret and not valid_proxy:
                raise HTTPException(
                    status_code=401, detail="operator proxy authentication failed"
                )
    ...
    def claim_job(
        ...
        try:
            ...
            if (registered_test_job and not remote_head_sha) or (
                remote_head_sha
                and remote_head_sha != str(claimed["head_sha"]).lower()
            ):
            ...
            try:
                remote_run_matches = (
                    remote_run_id is not None
                    and int(remote_run_id) == int(claimed["run_id"])
                )
            except (TypeError, ValueError):
    ...
    async def put_artifact(
        ...
        maximum = (
            MAX_REPORT_BYTES
            if report_like
            else min(settings.max_artifact_bytes, 250 * 1024 * 1024)
        )
        ...
        if existing_test_run is not None and normalized_report is not None:
            existing_execution = {
            ...
            existing_result = {
                "status": existing_test_run.get("test_status"),
        ...
        if normalized_report is not None:
            ...
            try:
                ...
                if is_test_result:
                    ...
                    row = store.merge_test_run_sources(job_id, suite, attempt) or row
                else:
                    row = store.merge_test_run_sources(job_id, suite, attempt)
