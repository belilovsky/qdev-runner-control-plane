...
def _coverage(value: Any) -> list[dict[str, Any]]:
    ...
    for entry in entries:
        ...
        if not math.isfinite(float(percentage)):
            raise TestReportError("coverage.percentage must be finite")
...
def _result(value: Any) -> dict[str, Any]:
    ...
    if executed > total or failed > executed or skipped > total or executed + skipped != total:
...
def normalize_test_run(
    ...
    if result["status"] == "passed" and (execution_status != "completed" or finished_at is None):
        raise TestReportError("a passed result requires completed execution")
    ...
    for line in lines:
        try:
            if line.startswith("LF:"):
                value = int(line[3:])
                if value < 0:
                    raise TestReportError("invalid LCOV counter")
                total += value
                found = True
...
def parse_cobertura(
    ...
    if not math.isfinite(rate) or not 0 <= rate <= 1:
        ...
        if denominator < 0 or covered < 0:
            raise TestReportError("invalid Cobertura counters")
        coverage = (
