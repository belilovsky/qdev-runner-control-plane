...
class Profile:
    ...
    # Native report formats that must be persisted before a successful test
    # receipt for this profile can be admitted as passed.
...
class RecoverySupersedeRequest(BaseModel):
    """Release an uninvoked recovery fence left by a superseded controller."""
...
class RecoveryOperationResponse(BaseModel):
    ...
    state: str = Field(
        pattern=(
            ...
            r"already_completed|not_applied|failed|ambiguous|superseded)$"
        )
    )
