...
Run complete provider CI for the exact candidate first (`scripts/verify_controller_ci.py`).
Hosted is normal; the existing manual workflow offers explicit owner recovery,
which is recorded as `controller-recovery`, never hosted. Local checks alone
are not provider acceptance. Stage the exact `git archive` in its own directory
below `/opt/qdev-runner-control-plane/releases/` with the manifest generated by
`scripts/controller_release_guard.py manifest`. The externally recorded SHA
and digest must match every file and mode; no dirty checkout, symlinks or
unbound additions are accepted. This manifest is a source binding, not a signature.

Get a fresh native operator `release-audit` and retain its expected revision
and digest. Run the staged revision's reviewed native transaction as root on
the controller host (SSH is only transport; credentials stay on that host):

```bash
sudo python3 -B scripts/release_controller_exact.py \
  --release /opt/qdev-runner-control-plane/releases/EXACT_SHA \
  --revision EXACT_SHA --digest SOURCE_DIGEST \
  --expected-revision CURRENT_SHA --expected-digest CURRENT_DIGEST \
  --transaction-id controller-release-UNIQUE_ID
```

The transaction requires at least 30 GiB free disk, at most 85% disk use, at
...
configuration is retained with actual immutable broker image IDs in a durable,
mode-0700 transaction directory. Candidate and previous images must both open
an isolated migrated SQLite backup without changing any original columns or
queue rows. The live database is never directly rewritten/restored. It does
not restart a worker, stop the registry, remove Compose or Docker
objects, or touch product containers. If either Compose or the public health
check fails, the transaction restores the previous release, configuration and
exact images, unless a foreign runtime/configuration change requires reconciliation.
...
The lower-level activation script is internal to this transaction, not an
alternate release path. Old environment-only capacity overrides do not bypass
the transaction's build gate. A measured owner-authorized disk exception must
be implemented as a bounded transaction policy before use; memory/CPU gates
must not be lowered. Worker admission overrides are separate signed scopes.

Success requires fresh enforced v2 worker and release audits verified by the
native mTLS operator, correct source digest, current link and running image IDs.
An interruption is resumed using the same transaction ID and identical inputs:
a completed activation is verified, not deployed again; a partial activation
is reconciled or rolled back. A rolled-back transaction cannot deploy again.
Rollback uses its retained image IDs and configuration, never mutable image tags:

```bash
sudo scripts/rollback_controller_release.sh controller-release-UNIQUE_ID
```
