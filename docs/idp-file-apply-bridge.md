...
## Fixed one-shot host entrypoint

The existing `qdev_admin_platform_release_host_agent.py` now has an `idp`
subcommand: `intake|apply|inspect|reconcile|observe --transaction <name>`.
`apply` additionally requires `--ci ci-<16 hex>.json`; intake does not accept CI.
There are no selectable target, state root, profile, command or configuration
arguments. This is source implementation, **not installed production authority**.

The compiled scope is `id-qdev-run`, `belilovsky/id-qdev-run`,
`qdev-release-idp`, `idp-file-v1`, artifact prefix `qdev/idp-release`, and actual
host **srv1380923** (not QMT's `srv138jump`). Execution requires root on that
hostname, plus the fixed private configuration `/etc/qdev-release-agents/idp.env`
and the matching `qdev-host-agent:srv1380923` mTLS identity/signing material.
Hostname is a routing restriction, not an authentication proof. This source
profile does not register a lane, enroll a certificate, issue a claim or modify
controller policy. Its 1 GiB profile floor is not operational admission: the
native transaction still measures each affected filesystem, rollback/restore
peak requirements and its reserve immediately before the guarded operation.

The configuration reuses the six existing `QDEV_RELEASE_*` keys. Its own file,
certificate/key/CA and dispatch-secret references use descriptor-based no-follow
ancestry traversal, private `0700` parents, root-owned `0600` single-link regular
files and bounded stable reads. Duplicate keys, endpoint userinfo, a foreign
host/port, relative paths and unsafe ancestry are rejected without printing
credential data. No secret is an argument, generated by this entrypoint or
included in its response.

For intake, the existing native CI fetch/upload path must place the original
outer archive as `artifact.tar.gz` and the controller-issued signed job as
`controller-job.json` in `/var/lib/qdev-idp/releases/<transaction>/`. The job is
verified before reading a new large archive or contacting the private endpoint.
Both files remain private; the complete candidate is fetched through the existing
authenticated endpoint, not accepted as caller assertions. Full archive and
component checks precede immutable retention. Exact intake replay compares both
staged inputs with retained bytes and finishes interrupted directory sync without
network access or renewed authority, even after claim expiry. Conflicting inputs
are rejected; intake does not load helpers or constitute deployment.

The remaining actions read the retained publication and enter the existing
verified native invocation. An initial `inspect` works with no staged upload or
publication and returns `inputs_not_published`, without executing a helper.
IdP is deliberately excluded from generic `PROFILES`/`run_once`: the generic
poller takes the host lock first and has automatic rollback semantics. This mode
takes no outer host lock, polls no job, starts no service, retries no operation
and performs no automatic rollback. Native-global then host-journal lock order
and separate fresh provider/controller gates remain mandatory.

Entrypoint tests use real private files, signatures, both archive layers and
retention with synthetic authority. They cover exact/expired retry, post-rename
sync failure, missing initial input, contradictory publication, private-path and
configuration failures, routing restrictions, no polling/outer host lock,
redaction and unknown native outcome without retry. They do not prove production
installation, enrollment, a live controller claim or live acceptance.
...
2. Install the fixed IdP invocation through the native verified-helper/global-lock
   ...
   factory, immutable retention, restart entrypoint, authenticated full-candidate
   intake and fixed one-shot CLI above are implemented, but the release owner
   must still fetch/upload the published outer archive and signed job through
   the native CI path and call that CLI for its exact enrolled target.
   Native reconciliation is now connected to its existing pending host journal
   in the native invocation; the installed release owner must use it. The CLI is
   not yet installed/enrolled on production and is not a second polling owner.
