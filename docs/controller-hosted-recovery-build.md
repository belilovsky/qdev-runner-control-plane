...
scans never produce usable recovery material. If QDev-store delivery is unavailable,
the job records that transport failure but succeeds after retaining the scanned GitHub
artifact; the artifact still has no recovery identity until reconciliation succeeds.
...
When the QDev delivery is unavailable because the running controller cannot yet
