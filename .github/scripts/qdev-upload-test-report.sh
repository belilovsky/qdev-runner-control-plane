...
# Stream native test evidence; qdev-runner-control-plane owns parsing and
# validation.  This helper never creates credentials or exposes them to the
# browser.
...
upload_report() {
  local report_path="$1" report_format="$2"
  test -f "$report_path"
  local report_name="${report_path##*/}"
  test -n "$report_name"
  local sha256
  sha256="$(shasum -a 256 "$report_path" | awk '{print $1}')"
  local attempt="${QDEV_TEST_ATTEMPT:-1}" suite="${QDEV_TEST_SUITE:-}"
...
if (( uploaded == 0 )); then
  echo "one of QDEV_TEST_REPORT, QDEV_TEST_JUNIT, QDEV_TEST_LCOV or QDEV_TEST_COBERTURA is required" >&2
  exit 2
fi
